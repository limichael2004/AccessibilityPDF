...
## About the Project
I created AccessibilityPDF to explore how coding and computer vision for individuals with fine motor limitations. For many people with conditions that limit their extremities, standard input devices like mice and keyboards create barriers to basic tasks like reading. This project aims to remove those barriers by turning a standard webcam into a input controller. It allows users to navigate, zoom, and read PDF documents entirely through facial expressions and head movements.

## How it Works
The application uses the MediaPipe library to track facial geometry in real-time. It maps  physical actions—like turning the head, raising eyebrows, or smiling—to digital commands within the PDF reader.
...
2. Privacy: All processing happens locally on your computer. No video feed or biometric data is ever stored.

3. Adaptive Configuration: I realized that "accessibility" means something different for everyone. The software includes a configuration "suite" that allows users to remap every gesture and adjust sensitivity thresholds. This ensures the tool adapts to the user's specific range of motion and comfort level, rather than forcing the user to adapt to the tool.
